...
public class Calcu extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        PanelDatos.add(BtoPotencia, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 10, 50, 50));
    ...
    private void BtoPotenciaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtoPotenciaActionPerformed
        try {
            numeros[0] = Double.parseDouble(txtOperacion.getText());
            txtOperacion.setText("");
            exponencial = true;
            decimal = false;
        } catch (Exception e) {
    ...
    private void BtoResActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtoResActionPerformed
        ...
        decimal = false;
        try {
            numeros[1] = Double.parseDouble(txtOperacion.getText());
            if(division == true){
                if(numeros[1] == 0){
            ...
            }else if(exponencial == true){
                resultado = pow(numeros[0],numeros[1]);
                if (resultado %1 == 0) {
                        entero = Double.toString(resultado);
                        txtOperacion.setText(numeros[0]+"^"+numeros[1]);
                        txtResultado.setText(entero.substring(0, entero.length()-2));
